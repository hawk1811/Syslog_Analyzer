...
// StartHeartbeat starts the heartbeat for HEC destinations
func (t *Tester) StartHeartbeat() {
	t.heartbeatTicker = time.NewTicker(5 * time.Minute)
	go func() {
		for {
			select {
...
func (t *Tester) StopHeartbeat() {
	if t.heartbeatTicker != nil {
		t.stopHeartbeat <- struct{}{}
	}
}

// sendHeartbeats sends heartbeat to all HEC destinations
func (t *Tester) sendHeartbeats() {
	// Get all sources with HEC destinations
	sources := t.getSourcesWithHEC()

	for _, source := range sources {
		for _, dest := range source.Destinations {
			if dest.Type == "hec" && dest.Enabled {
				// Send heartbeat
				success, _ := t.testHECDestination(&dest, source.Name, source.IP)
				if !success {
					log.Printf("Warning: Heartbeat failed for source %s, HEC destination %s", source.Name, dest.Name)
				}
			}
		}
	}
...
// getSourcesWithHEC returns all sources that have HEC destinations
func (t *Tester) getSourcesWithHEC() []models.SourceConfig {
	// This is a placeholder - you'll need to implement the actual source retrieval
	// from your configuration system
...
func (t *Tester) testHECDestination(dest *models.Destination, sourceName, sourceIP string) (bool, string) {
	...
	var avgDailyGB string = "not_started"
	if sourceMetrics, ok := t.getSourceMetrics(sourceName); ok {
		if sourceMetrics.DailyAvgGB > 0 {
			avgDailyGB = fmt.Sprintf("%.2f", sourceMetrics.DailyAvgGB)
		}
	}
	...
	testPayload := map[string]interface{}{
		...
		"event": map[string]interface{}{
			...
			"Avg_daily_ingested_GB": avgDailyGB,
		},
...
// getSourceMetrics retrieves metrics for a source
func (t *Tester) getSourceMetrics(sourceName string) (*models.SourceMetrics, bool) {
	// This is a placeholder - you'll need to implement the actual metrics retrieval
	// from your metrics tracking system
	return nil, false
}
