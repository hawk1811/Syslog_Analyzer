...
func (s *Server) handleAPIReport(w http.ResponseWriter, r *http.Request) {
	...
	pdf.Ln(6)
	pdf.Cell(40, 6, fmt.Sprintf("Total Hourly Avg GB: %.5f", globalMetrics.TotalHourlyAvg.HourlyAvgGB))
	pdf.Ln(6)
	pdf.Cell(40, 6, fmt.Sprintf("Total Daily Avg GB: %.5f", globalMetrics.TotalDailyAvg.DailyAvgGB))
	pdf.Ln(12)
